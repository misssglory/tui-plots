...
pub fn format_times(ts_vec: &[f64]) -> Vec<String> {
  ...
  let formatted: Vec<String> =
    ts_vec.iter().map(|&ts| format_time(ts)).collect();
...
pub fn pct_span(pct: Option<f64>) -> Span<'static> {
  match pct {
    Some(v) => {
      Span::styled(format!("{:+.1}%", v), Style::default().fg(pct_color(v)))
    }
...
pub fn format_y_labels(
  ...
  match scale_mode {
    ScaleMode::Linear => {
      levels.into_iter().map(|v| Line::from(Span::raw(format_sci(v)))).collect()
    }
    ScaleMode::Log10 => {
...
pub fn format_ratio(num: u64, den: u64) -> String {
  if den == 0 { "n/a".to_string() } else { format_sci(num as f64 / den as f64) }
}
...
pub fn project_to_cell(
  ...
  let col = plot_area.x
    + ((xr * (plot_area.width.saturating_sub(1) as f64)).round() as u16);
  let row = plot_area.y + plot_area.height.saturating_sub(1)
