...
impl CommandPalette {
    pub fn sync_with_selection(&mut self, cfg: &CommandConfig) {
        ...
        let defaults: Vec<String> = spec.args.iter().map(|a| a.default_value.clone()).collect();
...
pub fn build_predefined_command_json(
    ...
    for (idx, arg) in spec.args.iter().enumerate() {
        let raw = inputs
            .get(idx)
            .map(|s| s.as_str())
            .unwrap_or(arg.default_value.as_str());
...
fn parse_arg_value(arg_type: &ArgType, raw: &str) -> Result<serde_json::Value, String> {
    match arg_type {
        ArgType::Int => {
            let v = raw
                .trim()
